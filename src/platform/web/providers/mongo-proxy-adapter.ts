...
export class MongoProxyAdapter implements ProviderAdapter {
  private readonly endpoint: string;
  private readonly target: MongoProxyTarget;
  private readonly password: string;
  private readonly accessToken: string;

  constructor(target: MongoProxyTarget, password: string, accessToken: string) {
  ...
  private async request<T>(path: string, payload: Record<string, unknown>): Promise<T> {
    ...
    try {
      ...
      throw new Error(
        "Unable to reach MongoDB proxy from browser mode. Verify proxy endpoint and network reachability.",
      );
    }
    ...
    if (!response.ok) {
      throw new Error(
        await parseErrorMessage(
          response,
          `MongoDB proxy request failed with status ${response.status}.`,
        ),
      );
    }
